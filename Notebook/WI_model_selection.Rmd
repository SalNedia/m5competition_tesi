...
In questo caso viene analizzata la serie di secondo livello relativa allo stato WI
...
```{r}
df_WI<-data.frame(df_sales_states_train$`df_calendar$date`,df_sales_states_train$WI)
names(df_WI)<-c("Date","Y")
list_of_frames <- expanding(df_WI, 1240, step=28) #1210=circa 3 anni e mezzo
...
df_WI <- cbind(df_WI,df_calendar_train$wday,df_calendar_train$month,df_calendar_train$snap_CA,df_calendar_train$snap_TX,df_calendar_train$snap_WI)
df_WI
```
## NARX
